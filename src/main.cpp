...
int main() {
    ...
    for (int platformIndex = 0; platformIndex < platformsCount; ++platformIndex) {
        ...
        cl_int result = clGetDeviceIDs(platform, CL_DEVICE_TYPE_GPU, 0, nullptr, &devicesCount);
        if (result == CL_DEVICE_NOT_FOUND || devicesCount == 0) {
            continue;
        }
    ...
    if (device == 0) {
        for (int platformIndex = 0; platformIndex < platformsCount; ++platformIndex) {
            ...
            cl_uint devicesCount = 0;
            cl_int result = clGetDeviceIDs(platform, CL_DEVICE_TYPE_CPU, 0, nullptr, &devicesCount);
            if (result == CL_DEVICE_NOT_FOUND || devicesCount == 0) {
                continue;
            }
            OCL_SAFE_CALL(result);
    ...
    if (device == 0) {
        ...
        std::cout << "Running on device: " << deviceName.data() << std::endl;
    }
    ...
    cl_mem as_gpu = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, n * sizeof(float), as.data(), &errcode);
    OCL_SAFE_CALL(errcode);
    cl_mem bs_gpu = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, n * sizeof(float), bs.data(), &errcode);
    OCL_SAFE_CALL(errcode);
    cl_mem cs_gpu = clCreateBuffer(context, CL_MEM_WRITE_ONLY, n * sizeof(float), nullptr, &errcode);
    OCL_SAFE_CALL(errcode);
    ...
    cl_program program = clCreateProgramWithSource(context, 1, &kernel_sources_ptr, nullptr, &errcode);
    OCL_SAFE_CALL(errcode);

    // TODO 8 Теперь скомпилируйте программу и напечатайте в консоль лог компиляции
    // см. clBuildProgram
    cl_int buildResult = clBuildProgram(program, 1, &device, "", nullptr, nullptr);
